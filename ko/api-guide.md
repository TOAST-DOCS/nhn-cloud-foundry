...
!!! tip "알아두기"
    ...
    학습에도 조건이 있습니다. 학습은 시계열 단위로 나누어 교차 검증하므로 시계열이 하나뿐이면 학습이 실패하고, 다섯 개보다 적으면 있는 수만큼만 나누어 학습합니다. 또한 시계열마다 약 4시간 이상 끊김 없이 쌓여야 학습에 쓰는 구간이 채워지며, 그보다 짧으면 있는 데이터를 반복해 채우므로 정확도가 떨어집니다.
...
!!! tip "알아두기"
    데이터 소스에 그룹 라벨을 지정하지 않았으면 앱 생성이 끝날 때 데이터 소스 전체가 그룹 하나로 등록되므로 이 API를 쓰지 않아도 동작합니다. 그룹 라벨을 지정했으면 그룹이 저절로 등록되지 않으므로, 사용 시작 API로 대상 그룹을 등록해야 탐지 결과를 받을 수 있습니다. 콘솔에는 이 조작이 없습니다. 등록된 그룹과 상태는 콘솔 앱 상세의 **그룹 목록** 탭에서 확인합니다.
