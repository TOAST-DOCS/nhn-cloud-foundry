...
- 학습은 시계열 단위로 나누어 교차 검증합니다. 데이터 소스에 시계열이 하나뿐이면 학습이 실패하고, 다섯 개보다 적으면 있는 수만큼만 나누어 학습합니다.
- 시계열마다 약 4시간 이상 끊김 없이 쌓여야 학습에 쓰는 구간이 채워집니다. 그보다 짧으면 있는 데이터를 반복해 채우므로 탐지 정확도가 떨어집니다.
...
| 상태 | 그룹의 현재 상태 |
| 추론 상태 | 이 그룹의 추론이 정상으로 돌고 있는지. 상태 칼럼과는 다른 축. 값에 마우스를 올리면 판정 시각을 확인 |
