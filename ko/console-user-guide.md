...
화면에 표시되는 날짜와 시각은 모두 접속한 브라우저의 시간대 기준으로 표시됩니다.
...
| Prometheus API | 지표(시계열) 데이터를 수집 API로 실시간 적재합니다. 단변량 이상 탐지 앱의 입력으로 사용합니다. |

<a id="datasource.create.detail"></a>
#### 상세 설정(파일 업로드) { #datasource.create.detail }
...
#### 상세 설정(Prometheus API) { #datasource.create.detail.prometheus }
...
| labelHash | string | 시리즈 식별자(자동 계산) |
| groupHash | string | 그룹 식별자(자동 계산) |
...
| 항목 | 필수 | 설명 |
| --- | --- | --- |
| 시리즈 식별 라벨 | O | 하나의 시계열(시리즈)을 어떤 라벨 조합으로 구분할지 지정. **전체 라벨 사용**(기본) 또는 **직접 지정** 중 선택 |
| 그룹 라벨 | X | 시계열을 묶어 관리하는 단위. 지정한 라벨 값이 같은 시계열이 한 그룹이 됨. 비우면 데이터 소스 전체가 하나의 그룹 |

- 시리즈 식별 라벨: 시계열마다 따로 학습하고 추론합니다. **전체 라벨 사용**은 라벨 조합이 다르면 다른 시계열로 봅니다(대부분 이 설정으로 충분합니다). **직접 지정**은 지정한 라벨 값이 같은 데이터만 한 시계열로 묶고 나머지 라벨은 구분에 사용하지 않습니다.
...
!!! tip "알아두기"
    Prometheus API 데이터 소스에 데이터를 보내는 방법은 자세히 보기의 **수집 방법** 탭 또는 [API 가이드](../api-guide/#metrics.ingest.api)의 '지표 수집 API'를 참고합니다.
...
- 데이터 소스가 준비 완료(COMPLETED) 상태여야 설정할 수 있습니다. 그 밖의 상태에서는 **설정 불가** 안내와 함께 현재 상태가 표시됩니다.
- 활성화는 비동기로 진행되며, 진행 상황은 화면이 자동으로 갱신해 보여 줍니다.

!!! danger "주의"
    Event API를 활성화하면 스냅샷 업로드(CSV 파일 업로드, Ingest API 스냅샷)가 차단됩니다. 스냅샷을 업로드하려면 Event API를 먼저 비활성화해야 합니다.
    스냅샷 업로드 후 Event API를 다시 활성화하면 이벤트 수집 위치가 최신 오프셋으로 초기화됩니다.

이벤트를 전송하는 방법은 [API 가이드](../api-guide/#event.ingest.api)의 '이벤트 수집 API'를 참고합니다.
...
| 규칙 | 시각 단위, 라벨, 부가 정보, 값, 응답에 대한 유의 사항 |

자세한 요청 형식과 오류 응답은 [API 가이드](../api-guide/#metrics.ingest.api)의 '지표 수집 API'를 참고합니다.
...
| 단변량 이상 탐지 | 지표(시계열)를 하나씩 학습하여 정상 범위를 벗어난 값을 탐지합니다. |
...
| 사용 가능한 GPU가 없습니다. | GPU를 사용하는 앱을 추가로 만들 수 없음 |
...
#### 상세 설정(추천 시스템) { #app.create.detail }
...
#### 상세 설정(단변량 이상 탐지) { #app.create.detail.univariate }

단변량 이상 탐지 앱은 지표(시계열) 하나씩을 따로 학습하여 정상 범위를 벗어난 값을 탐지합니다. 상세 설정은 데이터 소스, 모델 자원, 재학습, 탐지 옵션, 결과 전송 순으로 구성됩니다.
...
| 항목 | 필수 | 설명 |
| --- | --- | --- |
| CPU 리밋 | O | 추론 서버에 배정하는 CPU 상한. Kubernetes 표기(예: 2, 500m) 사용. 최소 2 vCPU, 기본값 2 |
| 메모리 리밋 | O | 추론 서버에 배정하는 메모리 상한. Kubernetes 표기(예: 1Gi, 512Mi) 사용. 최소 1Gi, 기본값 1Gi |
...
| 항목 | 필수 | 설명 |
| --- | --- | --- |
| 재학습 주기 | X | 토글로 사용 여부를 지정(기본 끔). 켜면 아래 주기를 지정 |

| 주기 | 설정 항목 |
| --- | --- |
| 매일 | 시(0~23, 기본 3시), 분(10분 단위, 기본 0분) |
| 매주 | 요일, 시, 분 |
| 시간 간격 | 간격(1, 2, 3, 4, 6, 8, 12시간), 분 |
...
| 디바이스 | O | 추론과 학습에 사용할 연산 장치. CPU(기본) 또는 GPU |

전송 모드는 다음 중에서 선택합니다.

| 값 | 설명 |
| --- | --- |
| 정확 모드(기본) | 지표 준비(웜업)가 끝난 뒤부터 신뢰할 수 있는 값만 전송 |
| 즉시 모드 | 활성화 직후부터 전송. 준비가 끝나기 전 값은 참고용 |

- 새로 들어온 지표는 판정에 사용할 데이터가 쌓이고 임계값이 그 지표의 분포로 보정되기까지(웜업) 수 시간이 걸립니다.
- 점수 스케일을 켜면 점수를 0~1로 압축한 뒤 100을 곱해 0~100으로 내보내고, 임계값도 같은 스케일로 계산합니다. %(백분율) 축을 사용하는 대시보드와 스케일을 맞출 때 사용합니다. 끄면 원값(raw)을 그대로 내보냅니다.
- 추론과 학습에는 같은 디바이스가 적용됩니다. 대부분 CPU로 충분하며, GPU는 서비스 환경의 리소스 상황에 따라 선택하지 못할 수 있습니다.

!!! danger "주의"
    점수 스케일은 앱을 생성한 뒤에는 변경할 수 없습니다.

<a id="app.create.detail.univariate.sink"></a>
##### 결과 전송(Prometheus) { #app.create.detail.univariate.sink }
...
!!! tip "알아두기"
    추론 결과는 Prometheus 전송과 별개로 결과 데이터 소스에 항상 저장됩니다. 결과 데이터 소스는 앱을 생성할 때 자동으로 만들어지며(타입: 단변량 이상 탐지 결과), 분석 메뉴에서 조회할 수 있습니다.
...
context에 다음 키를 추가하면 사용자의 행동 신호를 입력할 수 있습니다. 행동 신호는 추천 결과의 순서를 조정하는 데 사용됩니다.

| 키 | 설명 | 유형 |
| --- | --- | --- |
| impressions | 사용자에게 노출한 아이템 목록 | 없음 |
| interactions | 사용자가 아이템에 보인 반응 | CLICK, CONVERSION |
...
!!! tip "알아두기"
    ...
    impressions는 추천 응답을 받을 때마다 자동으로 한 건씩 쌓입니다. 사용자를 변경하거나 화면을 새로 고치면 초기화되며, 행의 ✕로 하나씩, 필드의 ✕로 전체를 지울 수 있습니다.
...
### 앱 상세(단변량 이상 탐지) { #app.detail.univariate }
...
| 항목 | 설명 |
| --- | --- |
| 전송 주소 | 결과를 전송하는 주소(전송 URL + API 경로) |
...
| 칼럼 | 설명 |
| --- | --- |
| 그룹 키 | 그룹을 나누는 기준이 되는 라벨 이름. 그룹 키 필드가 없는 앱은 `-` |
...
- 상태 필터와 검색(그룹 키 또는 그룹 해시)으로 목록을 좁힐 수 있습니다.
...
- 해시가 예상과 다르면 정규형(이름 순으로 정렬해 쉼표로 이은 값)부터 대조합니다.
