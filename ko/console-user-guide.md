...
서비스 활성화는 [1:1 문의](https://www.nhncloud.com/kr/support/inquiry)로만 신청할 수 있습니다. 원하는 리소스 크기를 포함해서 문의하세요. 신청한 클러스터가 생성되면 데이터 소스 생성부터 순서대로 사용할 수 있습니다.
...
| 값 | 설명 |
| --- | --- |
| 확인 전 | 반영 여부를 아직 확인하지 않은 상태. 앱을 만든 직후에는 자동으로 확인되어 반영됨으로 시작 |
| 반영 중 | 변경한 설정을 반영하는 중 |
| 반영됨 | 변경한 설정이 반영 완료 |
...
| 상태 | 그룹의 현재 상태 |
| 추론 상태 | 이 그룹의 추론이 정상으로 돌고 있는지. 상태 칼럼과는 별개로 표시. 값에 마우스를 올리면 판정 시각을 확인 |
