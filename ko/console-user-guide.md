...
## Machine Learning > NHN Cloud Foundry > 콘솔 사용 가이드 { #foundry.console.guide }
...
| 항목 | 필수 | 설명 |
| --- | --- | --- |
| 데이터 소스 이름 | O | 영문, 숫자, -, _ 사용(1~255자) |
| 테이블 이름 | O | 영문 소문자, 숫자, _ 사용(1~255자) |
| 설명 | X | 데이터 소스 설명 |
...
| 기본 키 필드 | X | 쉼표로 구분하여 입력(예: user_id,item_id) |
...
2. 우측 **설정 패널**에서 기본 정보를 입력합니다.
    ...
    | 항목 | 필수 | 설명 |
    | --- | --- | --- |
    | 파이프라인 이름 | O | 파이프라인을 식별하는 이름(최대 255자) |
...
!!! danger "주의"
    파이프라인을 실행하려면 반드시 1개 이상의 DATASET 노드가 존재해야 합니다.
...
변환 작업마다 노드 이름(최대 30자, 영문·숫자·`-`·`_`·한글)을 지정하며, 한 노드에 여러 변환 작업을 추가하면 순서대로 연결된 노드가 생성됩니다.
...
| 항목 | 필수 | 설명 |
| --- | --- | --- |
| 앱 이름 | O | 앱을 식별하기 위한 이름(영문, 숫자, 하이픈만 허용) |
| 앱 설명 | O | 앱 설명 |
| 앱 유형 | O | **추천 시스템** 선택 |
...
| Warm User (Graph) | 유저가 인터랙션한 아이템과 유사한 아이템을 그래프 관계 기반으로 추천합니다. |
...
앱 ID, 앱 이름, 상태, 앱 유형, 설명, 생성일, 수정일, 버전을 확인할 수 있습니다.
